...
df['target'] = df['着_順'].apply(lambda x: 1 if x <= 3 else 0).astype(int)
...
columns_to_drop = [
    'index' , '予_想', '総_評', '着_順', '選手名', 'レース名', '開催日'] #'レースタイトル',
...
save_model(final_model, 'keirin_prediction_model_combined')
print(f"\nモデルが 'keirin_prediction_model_combined.pkl' として保存されました。")

# --- 5. 評価と診断 ---
# 正しい列の順序を保存
correct_columns = data_train.drop('target', axis=1).columns.tolist()
pd.to_pickle(correct_columns, 'model_columns.pkl')
print("モデルの列順序を 'model_columns.pkl' に保存しました。")
