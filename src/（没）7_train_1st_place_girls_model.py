...
df_raceinfo = pd.read_pickle('race_info2_girls_202106-202512.pkl')
...
# 【変更点④】保存するモデル名を変更
save_model(final_model, 'keirin_model_1st_place_girls') # ← 新しい名前を指定
print(f"\n1着予測モデルが 'keirin_model_1st_place_girls.pkl' として保存されました。")

# --- 5. 評価と診断 ---
# 正しい列の順序を保存
correct_columns = data_train.drop('target', axis=1).columns.tolist()
# 【変更点⑤】列順序ファイルも別名で保存
pd.to_pickle(correct_columns, 'model_columns_1st_place_girls.pkl')
print("1着予測モデルの列順序を 'model_columns_1st_place_girls.pkl' に保存しました。")
