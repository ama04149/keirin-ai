...
if __name__ == '__main__':
    ...
    INPUT_CSV = 'keirin_prediction_result_oana.csv'
    OUTPUT_CSV = 'keirin_oana_summary.csv'
    # --------------------------------
    ...
    race_summary = df_sorted.groupby(['競輪場', 'レース番号', 'レース番号_数値']).apply(process_race2).reset_index()
    ...
    # 新しいCSVファイルとして保存
    output_filename = 'keirin_oana_summary.csv'
    ...
    df['3連単_3着以内'] = df['3連単_3着以内'].str.replace('="', '').str.replace('"', '')
     ...
     # 変更した条件に基づいてレコードを抽出
    matched_df = df[df.apply(check_match, axis=1)].copy()

    ## 余分な列を削除
    #matched_df = matched_df.drop(columns=['3連単_1着_ソート済', '3連単_3着以内_ソート済'], errors='ignore')

    # 結果を表示
    print("--- 3連単_1着と3着以内_補欠を含めた4つの数値のうち、3つが一致したレコード ---")
    print(matched_df)   
    ...
    matched_df.to_csv(output_filename4, encoding='utf-8-sig', index=False)
    ...
    files_to_upload = [OUTPUT_CSV, output_filename2, output_filename3, output_filename4,"index.html"]  # ← ここに追加
