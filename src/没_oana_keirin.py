...
# pycaret_keirin.py をコピー、予想印を学習対象から除外して作成

# --- 1. データの読み込みと結合 ---
df_shussou = pd.read_pickle('race_card3_202106-202507.pkl')
df_raceinfo = pd.read_pickle('race_info2_202106-202507.pkl')
...
df['target'] = df['着_順'].apply(lambda x: 1 if x <= 3 else 0)
...
#best_model2 = create_model(best_model, monotone_constraints=monotone_constraints)
...
save_model(final_model, 'keirin_oana_model')
print(f"\nモデルが 'keirin_oana_model.pkl' として保存されました。")

# --- 5. 評価と診断 ---
# 正しい列の順序を保存
correct_columns = data_train.drop('target', axis=1).columns.tolist()
pd.to_pickle(correct_columns, 'model_oana_columns.pkl')
print("モデルの列順序を 'model_oana_columns.pkl' に保存しました。")
